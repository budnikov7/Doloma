...
Процедура КорпусаКорпусКормленияПриИзменении(Элемент)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ЗаполнитьПоказателиКорпуса(ТекущиеДанные);
...
Процедура ПерезаполнитьКорпуса(Команда)
	...
	Для каждого Строчка Из Объект.Корпуса Цикл
		ЗаполнитьПоказателиКорпуса(Строчка);
		РассчитатьПоказателиКормления(Строчка, Истина);
	КонецЦикла;
...
&НаКлиенте
Процедура РассчитатьПоказателиКормления(ТекКорпус = Неопределено, Перезаполнить = Ложь)
	...
	Если (НЕ ЗначениеЗаполнено(ТекРаздача.Рацион) ИЛИ Перезаполнить) И ЗначениеЗаполнено(ТекКорпус.Рацион) Тогда
	...
	Если РеквизитыРациона.Вес = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю("Ошибка расчета коэффициента кормления. Вес рациона равен 0.");
	КонецЕсли;
	
	Если ТекКорпус.КоличествоГолов = 0 Тогда
		ЕстьОшибкиЗаполнения = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю("Ошибка расчета коэффициента кормления. Количество голов корпуса равно 0.");
	КонецЕсли;
	
	Если ТекКорпус.ПроцентСуточногоКормления = 0 Тогда
		ЕстьОшибкиЗаполнения = Истина;
		ОбщегоНазначенияКлиент.СообщитьПользователю("Ошибка расчета коэффициента кормления. Процент суточного кормления корпуса равен 0.");
	КонецЕсли;
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю("Ошибка расчета процента загрузки. Максимальная загрузка кормораздатчика равна 0.");
	КонецЕсли;
	
	РассчитатьЗагрузкуМиксера(ТекРаздача);
	
КонецПроцедуры

&НаКлиенте
Процедура РассчитатьЗагрузкуМиксера(ТекРаздача)
...
КонецПроцедуры // РассчитатьЗагрузкуМиксера()

&НаКлиенте
Процедура ЗаполнитьПоказателиКорпуса(ТекущиеДанные)
