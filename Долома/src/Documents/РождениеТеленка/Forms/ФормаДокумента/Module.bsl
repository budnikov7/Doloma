...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЖивотные
&НаКлиенте
Процедура ЖивотныеМатьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВзаимодействиеКлиент.ОткрытьФормуВыбораЖивотного(ЭтотОбъект, СписокПараметров);
...
Процедура ПроверкаВозможностиРедактирования(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ферма)  Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(,"Для редактирования данных необходимо заполнить Ферму");
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьКличкуТеленка()
...
&НаКлиенте
Процедура ПроверкаАвтонумерацииФермы()
