...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	Если НЕ ЗначениеЗаполнено(Объект.РежимРасчета) Тогда
		Объект.РежимРасчета = Перечисления.РежимыРасчетаРационов.Ручной;
	КонецЕсли;
...
Процедура Авторасчет(Команда)
	...
	// Проверяем, что режим расчета - автоматический
	Если Объект.РежимРасчета <> ПредопределенноеЗначение("Перечисление.РежимыРасчетаРационов.Автоматический") Тогда
		Вопрос = Новый СписокЗначений;
		Вопрос.Добавить("Да", "Да");
		Вопрос.Добавить("Нет", "Нет");
		
		Ответ = Вопрос.ВыбратьЭлемент("Режим расчета установлен как ""Ручной"". Переключить на ""Автоматический"" и продолжить?");
		Если Ответ = Неопределено ИЛИ Ответ.Значение = "Нет" Тогда
			Возврат;
		КонецЕсли;
		
		Объект.РежимРасчета = ПредопределенноеЗначение("Перечисление.РежимыРасчетаРационов.Автоматический");
	КонецЕсли;
	
	// Проверяем заполнение ограничений
	ЕстьОшибки = Ложь;
	Для Каждого СтрокаКорма Из Объект.Корма Цикл
		Если СтрокаКорма.МаксВес = 0 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон("Для корма ""%1"" не задан максимальный вес", СтрокаКорма.Номенклатура);
			Сообщение.Сообщить();
			ЕстьОшибки = Истина;
		КонецЕсли;
	КонецЦикла;
	...
	// Выполняем авторасчет
	РезультатРасчета = ВыполнитьАвторасчетНаСервере();
	
	Если РезультатРасчета.Успех Тогда
		// Применяем рассчитанные веса
		ПрименитьРассчитанныеВеса(РезультатРасчета.РассчитанныеВеса);
		...
		Если РезультатРасчета.КрасныеПоказатели.Количество() > 0 Тогда
			ТекстПоказателей = "";
			Для Каждого Показатель Из РезультатРасчета.КрасныеПоказатели Цикл
				Если ТекстПоказателей <> "" Тогда
					ТекстПоказателей = ТекстПоказателей + ", ";
				КонецЕсли;
				ТекстПоказателей = ТекстПоказателей + Строка(Показатель);
			КонецЦикла;
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Показатели вне нормы: " + ТекстПоказателей;
			Сообщение.Сообщить();
		КонецЕсли;
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Ошибка авторасчета: " + РезультатРасчета.Сообщение;
		Сообщение.Сообщить();
	КонецЕсли;
...
Функция ВыполнитьАвторасчетНаСервере()
	...
	// Получаем таблицу питательности
	ТаблицаПитательности = ПолучитьТаблицуПитательности(Объект.Корма.Выгрузить(,"Номенклатура,ВидКорма,Вес,Цена,МинВес,МаксВес"), Объект.Дата);
	
	// Получаем таблицу показателей
	ТаблицаПоказателей = Объект.ПоказателиРациона.Выгрузить(,"ПоказательРациона,ЕдиницаИзмерения,Минимум,Максимум");
	
	// Вызываем алгоритм оптимизации
	Результат = РасчетАвтоРациона.РассчитатьАвтоРацион(
		Объект.Корма.Выгрузить(,"Номенклатура,МинВес,МаксВес"),
		ТаблицаПитательности,
		ТаблицаПоказателей,
		Объект.Дата
	);
...
Процедура ПрименитьРассчитанныеВеса(РассчитанныеВеса)
	...
	Для Каждого СтрокаКорма Из Объект.Корма Цикл
		Вес = РассчитанныеВеса[СтрокаКорма.Номенклатура];
		Если Вес <> Неопределено Тогда
			СтрокаКорма.Вес = Вес;
			СтрокаКорма.Сумма = СтрокаКорма.Цена * Вес;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьВидимостьПолейРежима()
	...
	АвтоматическийРежим = (Объект.РежимРасчета = Перечисления.РежимыРасчетаРационов.Автоматический);
	
	// В автоматическом режиме показываем колонки МинВес и МаксВес, скрываем Вес
	// В ручном режиме наоборот
	Элементы.КормаВес.Видимость = НЕ АвтоматическийРежим;
	Элементы.КормаМинВес.Видимость = АвтоматическийРежим;
	Элементы.КормаМаксВес.Видимость = АвтоматическийРежим;
...
Процедура УстановитьОтборТаблицыПоказателей()
	...
	// Устанавливаем условное оформление для красных значений
...
Процедура УстановитьУсловноеОформлениеПоказателей()
	...
	// Добавляем условное оформление для показателей вне нормы
	// Условие 1: Содержание < Минимум
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = ЭлементУО.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ПоказателиРационаСодержание.Имя);
	
	ОтборЭлемента = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПоказателиРациона.Содержание");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше;
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПоказателиРациона.Минимум");
	
	ОтборЭлемента2 = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента2.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПоказателиРациона.Минимум");
	...
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПоказателиРациона.Максимум");
	
	ОтборЭлемента2 = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента2.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПоказателиРациона.Максимум");
	ОтборЭлемента2.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
