...
//    * СоответствиеПриоритетов - Соответствие - веса показателей для авторасчета (чем больше - тем важнее)
//
Функция ПодготовитьДанныеДляОптимизации(ДанныеРациона)
	...
	Результат.МассивПоказателей = ПодготовитьМассивПоказателей(
		ДанныеРациона.ТаблицаПоказателей,
		ДанныеРациона.ГруппаЖивотных,
		ДанныеРациона.Дата);
...
// Получает границы отклонений показателей из регистра сведений
//
// Параметры:
//  ГруппаЖивотных - СправочникСсылка.ГруппыЖивотных - группа животных
//  Дата - Дата - дата среза
//
// Возвращаемое значение:
//   ТаблицаЗначений - таблица с границами отклонений показателей
//
Функция ПолучитьГраницыОтклоненийПоказателей(ГруппаЖивотных, Дата)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НормативныеПоказателиРационов.ПоказательРациона КАК ПоказательРациона,
	|	НормативныеПоказателиРационов.НижняяГраницаОтклонения КАК НижняяГраницаОтклонения,
	|	НормативныеПоказателиРационов.ВерхняяГраницаОтклонения КАК ВерхняяГраницаОтклонения
	|ИЗ
	|	РегистрСведений.НормативныеПоказателиРационов КАК НормативныеПоказателиРационов
	|ГДЕ
	|	ГруппаЖивотных = &ГруппаЖивотных";
	
	Запрос.УстановитьПараметр("ГруппаЖивотных", ГруппаЖивотных);
...
//  ГруппаЖивотных - СправочникСсылка.ГруппыЖивотных - группа животных
//  Дата - Дата - дата среза для получения нормативов
//
// Возвращаемое значение:
//   Массив из Структура - массив показателей с параметрами
//
Функция ПодготовитьМассивПоказателей(ТаблицаПоказателей, ГруппаЖивотных, Дата)
	
	МассивПоказателей = Новый Массив;
	
	// Получаем границы отклонений из регистра НормативныеПоказателиРационов
	ТаблицаГраниц = ПолучитьГраницыОтклоненийПоказателей(ГруппаЖивотных, Дата);
	...
	Для каждого СтрокаПоказателя Из ТаблицаПоказателей Цикл
		...
		// Получаем границы отклонений для данного показателя
		НижняяГраницаОтклонения = -5; // Значение по умолчанию
		ВерхняяГраницаОтклонения = 5; // Значение по умолчанию
		
		СтрокаГраниц = ТаблицаГраниц.Найти(СтрокаПоказателя.ПоказательРациона, "ПоказательРациона");
		Если СтрокаГраниц <> Неопределено Тогда
			НижняяГраницаОтклонения = СтрокаГраниц.НижняяГраницаОтклонения;
			ВерхняяГраницаОтклонения = СтрокаГраниц.ВерхняяГраницаОтклонения;
		КонецЕсли;
		
		// Применяем нижнюю границу отклонения к минимуму (в процентах)
		Если ЗначениеЗаполнено(Показатель.Минимум) Тогда
			Показатель.Вставить("МинимумСПогрешностью", Показатель.Минимум * (1 + НижняяГраницаОтклонения / 100));
		Иначе
			Показатель.Вставить("МинимумСПогрешностью", 0);
		КонецЕсли;
		
		// Применяем верхнюю границу отклонения к максимуму (в процентах)
		Если ЗначениеЗаполнено(Показатель.Максимум) Тогда
			Показатель.Вставить("МаксимумСПогрешностью", Показатель.Максимум * (1 + ВерхняяГраницаОтклонения / 100));
		Иначе
			Показатель.Вставить("МаксимумСПогрешностью", 0);
		КонецЕсли;
...
// Возвращает соответствие весов показателей для авторасчета
//
// Возвращаемое значение:
//   Соответствие - соответствие имени показателя и его веса (чем больше - тем важнее)
//
Функция ПолучитьСоответствиеПриоритетовПоказателей()
	
	Веса = Новый Соответствие;
	
	// Получаем веса показателей из справочника ПоказателиРационов
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПоказателиРационов.ИмяПредопределенныхДанных КАК ИмяПредопределенныхДанных,
	|	ПоказателиРационов.ВесВАвторасчете КАК ВесВАвторасчете
	|ИЗ
	|	Справочник.ПоказателиРационов КАК ПоказателиРационов
	|ГДЕ
	|	ПоказателиРационов.ВесВАвторасчете > 0";
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ИмяПредопределенныхДанных) Тогда
			Веса.Вставить(Выборка.ИмяПредопределенныхДанных, Выборка.ВесВАвторасчете);
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого Показатель Из ДанныеДляОптимизации.МассивПоказателей Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Показатель.Минимум) И ФактическоеЗначение < Показатель.МинимумСПогрешностью Тогда
				...
				КонецЕсли;
			ИначеЕсли ЗначениеЗаполнено(Показатель.МаксимумСПогрешностью) И ФактическоеЗначение > Показатель.МаксимумСПогрешностью Тогда
				// Выше максимума с учетом погрешности - большой штраф
				Если Показатель.Максимум > 0 Тогда
					ШтрафЗаВыходЗаГраницы = (ФактическоеЗначение - Показатель.МаксимумСПогрешностью) / Показатель.Максимум * 5000;
				Иначе
					ШтрафЗаВыходЗаГраницы = (ФактическоеЗначение - Показатель.МаксимумСПогрешностью) * 500;
				КонецЕсли;
			КонецЕсли;
		...
		// Получаем вес показателя из справочника
		ВесПоказателя = ДанныеДляОптимизации.СоответствиеПриоритетов[ИмяПоказателя];
		Если ВесПоказателя = Неопределено Тогда
			ВесПоказателя = 1; // Минимальный вес по умолчанию
		КонецЕсли;
		
		// Коэффициент веса (чем больше вес, тем больше штраф)
		КоэффициентПриоритета = ВесПоказателя;
...
Функция ОценитьРешение(ВесаКормов, ДанныеДляОптимизации)
	...
	Для каждого Показатель Из ДанныеДляОптимизации.МассивПоказателей Цикл
		...
		Если ЗначениеЗаполнено(Показатель.Минимум) Тогда
			Если ФактическоеЗначение < Показатель.МинимумСПогрешностью Тогда
				ВнеНормы = Истина;
			КонецЕсли;
			Результат.ОбщееКоличествоПоказателей = Результат.ОбщееКоличествоПоказателей + 1;
		ИначеЕсли ЗначениеЗаполнено(Показатель.Максимум) Тогда
			Если ФактическоеЗначение > Показатель.МаксимумСПогрешностью Тогда
				ВнеНормы = Истина;
			КонецЕсли;
			Результат.ОбщееКоличествоПоказателей = Результат.ОбщееКоличествоПоказателей + 1;
		КонецЕсли;
...
// Сортирует показатели по весу в авторасчете
//
// Параметры:
//  МассивПоказателей - Массив - массив показателей
//  СоответствиеПриоритетов - Соответствие - веса показателей (чем больше - тем важнее)
//
// Возвращаемое значение:
//   Массив - отсортированный массив показателей (от большего веса к меньшему)
//
Функция СортироватьПоказателиПоПриоритету(МассивПоказателей, СоответствиеПриоритетов)
	...
	Для каждого Показатель Из МассивПоказателей Цикл
		...
		Вес = СоответствиеПриоритетов[Показатель.ИмяПредопределенныхДанных];
		Если Вес = Неопределено Тогда
			Вес = 1; // Минимальный вес по умолчанию
		КонецЕсли;
		
		НоваяСтрока.Приоритет = Вес;
	КонецЦикла;
	
	// Сортируем по убыванию веса (больший вес = выше приоритет)
