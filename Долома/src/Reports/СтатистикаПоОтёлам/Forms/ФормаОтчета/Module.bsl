...
Процедура ОткрытьОтчётСОтбором(ДополнительныеПараметры,НаименованиеВариантаОтчета,ИмяРесурса)
	...
	ИДНастройкиПГЗдесь = НастройкиЗдесь.ПараметрыДанных.Элементы.Найти("ПроизводственнаяГруппа").ИдентификаторПользовательскойНастройки;
	НайденныйПараметрПГ = НастройкиПользовательскиеЗдесь.Элементы.Найти(ИДНастройкиПГЗдесь);
	...
	КонецЕсли;  
	Если НайденныйПараметрПГ <> Неопределено Тогда
		ПараметрыФормы.Вставить("ПроизводственнаяГруппа", НайденныйПараметрПГ.Значение);
	КонецЕсли;  
