...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьВыборДействияЗавершение", ЭтотОбъект, ПоляРасшифровки);
...
&НаКлиенте
Процедура РаспечататьКарточки(Команда)
	ТабДок = РаспечататьКарточкиНаСервере();
	ТабДок.Показать("Печать карточек телят");
КонецПроцедуры

&НаСервере
Функция РаспечататьКарточкиНаСервере()
	
	КомпоновщикНастроек = Отчет.КомпоновщикНастроек;
	НастройкиОтчета = КомпоновщикНастроек.ПолучитьНастройки(); 

	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	ОтчетОбъект = Отчеты.СписокРожденныхТелят.Создать();
	МакетКомпоновки = КомпоновщикМакета.Выполнить(ОтчетОбъект.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных"), НастройкиОтчета,,,
	...
	Макет = ОтчетОбъект.ПолучитьМакет("КарточкаТелёнка"); 
	оШапка  = Макет.ПолучитьОбласть("Шапка");
	Сч = 0;  
	ПоследнийДеньВыпойкиТелят = ПоследнийДеньВыпойкиТелят();
	
	Для Каждого Строчка Из ТЗ Цикл
		Если НЕ ЗначениеЗаполнено(Строчка.Животное) Тогда Продолжить КонецЕсли;
		Если Сч > 0 Тогда
			табДок.ВывестиГоризонтальныйРазделительСтраниц();	
		КонецЕсли; 
		Сч = Сч + 1;  
		ДанныеОтца = ДанныеЖивотных.ПолучитьИндексыБыка(Строчка.Семя.Бык);
		оШапка.Параметры.Заполнить(Строчка); 
		оШапка.Параметры.Заполнить(ДанныеОтца); 
		оШапка.Параметры.НомерЖивотного 			= Строчка.Животное.Бирка;
		оШапка.Параметры.Кличка 					= Строчка.Животное.Кличка; 
		оШапка.Параметры.ПоследнийДеньВыпойкиТелят 	= Строчка.ДатаРождения + ПоследнийДеньВыпойкиТелят * 86400; 
...
Функция ПоследнийДеньВыпойкиТелят()

	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	МАКСИМУМ(СхемыКормленияИВыпойкиТелятСхема.НомерСтроки * 7) КАК ПоследнийДеньВыпойки
	|ИЗ
	|	Справочник.СхемыКормленияИВыпойкиТелят.Схема КАК СхемыКормленияИВыпойкиТелятСхема
	|ГДЕ
	|	СхемыКормленияИВыпойкиТелятСхема.МолокоВДень <> 0";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ПоследнийДеньВыпойки;	
	Иначе 
	    Возврат 0;
	КонецЕсли; 
