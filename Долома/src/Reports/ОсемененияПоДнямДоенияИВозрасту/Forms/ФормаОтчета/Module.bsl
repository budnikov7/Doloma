...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Для каждого ПолеРасшифровки Из СтруктураРасшифровки Цикл
		Если СтрНайти(ПолеРасшифровки.Ключ,"Возраст") Тогда
			ДополнительныеПунктыМеню.Добавить("Открыть отчет расшифровку");
			СтруктураРасшифровки.Вставить("ПроизводственнаяГруппа","Тёлка");
		ИначеЕсли СтрНайти(ПолеРасшифровки.Ключ,"КоличествоДойныхДней") Тогда 
			ДополнительныеПунктыМеню.Добавить("Открыть отчет расшифровку");
			СтруктураРасшифровки.Вставить("ПроизводственнаяГруппа","Корова");
		ИначеЕсли СтрНайти(ПолеРасшифровки.Ключ,"ДатаОсеменения") Тогда;
			ЗаменитьКлюч = Истина;
			ЗначениеКлюча = НачалоДня(ПолеРасшифровки.Значение);  
		ИначеЕсли СтрНайти(ПолеРасшифровки.Ключ,"КоличествоДней") Тогда;
			ЗаменитьКлюч = Истина;
			ЗначениеКлюча = ОбщегоНазначения.ДобавитьДни(Дата(1,1,1),ПолеРасшифровки.Значение);  
		КонецЕсли;	
	КонецЦикла; 

	Если ЗаменитьКлюч Тогда
		СтруктураРасшифровки.Удалить("ДатаОсеменения");
		СтруктураРасшифровки.Удалить("КоличествоДней");
		СтруктураРасшифровки.Вставить("ДатаОсемененияДляРасшифровки",ЗначениеКлюча);
	КонецЕсли;  
	...
	ИначеЕсли  ВыполненноеДействие = "Открыть отчет расшифровку" Тогда 
		Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
			...
			Для каждого ЭлементСтруктуры Из ДополнительныеПараметры Цикл  
				...
				КонецЕсли;
			КонецЦикла;
			ОткрытьОтчётСОтбором(СтруктураОтбора,"Таблица",ИмяРесурса);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьОтчётСОтбором(ДополнительныеПараметры,НаименованиеВариантаОтчета,ИмяРесурса)
	...
	ОткрытьФорму("Отчет.ОсемененияПоДнямДоенияИВозрасту.Форма.ФормаОтчета", ПараметрыФормы, , Новый УникальныйИдентификатор);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеВарианта = Неопределено Тогда
		УстановитьТекущийВариант("ДиаграммаПоДнямДоения");
	КонецЕсли;  
