...
Процедура ПередЗагрузкойПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки) 
	...
	КонецЕсли;  
	ПроизводственнаяГруппа = Неопределено;
	Если Параметры.Свойство("ПроизводственнаяГруппа",ПроизводственнаяГруппа) Тогда
		УстановитьПользовательскийПараметрСКД(Настройки, "ПроизводственнаяГруппа", ПроизводственнаяГруппа);
	КонецЕсли;  
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Для каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
		Если СтрНайти(ПолеРасшифровки.Ключ,"Количество") ИЛИ СтрНайти(ПолеРасшифровки.Ключ,"Дней") Тогда
...
Процедура ПоказатьВыборДействияЗавершение(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли  ВыполненноеДействие = "Открыть отчет расшифровку" Тогда 
		Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
			...
			Если ИмяРесурса = "КоличествоОтёлов" Тогда 
				//МассивКУдалению = Новый Массив;
				//Для каждого Элемент Из СтруктураОтбора Цикл
				//	Если Элемент.Ключ = "Ферма" Тогда
				//		СтруктураОтбора.Вставить("ФермаОтёлы",Элемент.Значение);
				//		МассивКУдалению.Добавить(Элемент.Ключ);
				//	Иначе
				//	Если Элемент.Ключ = "Месяц" Тогда
				//		СтруктураОтбора.Вставить("МесяцОтёла",Элемент.Значение);
				//		МассивКУдалению.Добавить(Элемент.Ключ);
				//	КонецЕсли;   
				//КонецЦикла; 
				//Для каждого Элемент Из МассивКУдалению Цикл
				//	СтруктураОтбора.Удалить(Элемент);
				//КонецЦикла; 
				ОткрытьОтчётСОтбором(СтруктураОтбора,"РасшифровкаОтёлов",ИмяРесурса);
			ИначеЕсли ИмяРесурса = "КоличествоВыбылоПослеОтёла" Тогда
				//МассивКУдалению = Новый Массив;
				//Для каждого Элемент Из СтруктураОтбора Цикл
				//    Если Элемент.Ключ = "Месяц" Тогда
				//		СтруктураОтбора.Вставить("МесяцВыбытия",Элемент.Значение);
				//		МассивКУдалению.Добавить(Элемент.Ключ);
				//	КонецЕсли;   
				//КонецЦикла; 
				//Для каждого Элемент Из МассивКУдалению Цикл
				//	СтруктураОтбора.Удалить(Элемент);
				//КонецЦикла; 
				ОткрытьОтчётСОтбором(СтруктураОтбора,"РасшифровкаВыбытий",ИмяРесурса);
			ИначеЕсли СтрНайти(ИмяРесурса,"КоличествоГолов")	Тогда  
				ОткрытьОтчётСОтбором(СтруктураОтбора,"РасшифровкаЗаболеваний",ИмяРесурса);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
