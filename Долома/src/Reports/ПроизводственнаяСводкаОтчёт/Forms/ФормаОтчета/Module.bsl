...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)   
	...
	Если ЗначениеВарианта = Неопределено Тогда
		УстановитьТекущийВариант("ПроизводствоЗаГод");
	КонецЕсли;  
...
Процедура ПередЗагрузкойПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки) 
	...
	Если Параметры.Свойство("ПериодРасшифровки",ПериодРасшифровки) Тогда
		УстановитьПользовательскийПараметрСКД(Настройки, "Период", ПериодРасшифровки);
	КонецЕсли;  
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Для каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
		Если СтрНайти(ПолеРасшифровки.Ключ,"ИспользованоДенежныхСредств") Тогда
...
Процедура ОткрытьОтчётСОтбором(ДополнительныеПараметры,НаименованиеВариантаОтчета,ИмяРесурса)
	...
	Если НайденныйПараметрПериод <> Неопределено Тогда
		Период = Новый СтандартныйПериод(НайденныйПараметрПериод.Значение.ДатаНачала, НайденныйПараметрПериод.Значение.ДатаОкончания);
		ПараметрыФормы.Вставить("ПериодРасшифровки", Период);
	КонецЕсли;  
